...
Usage:
    ...
    - Note: Note that you have first to run `python GitHubRepositoryFetcher.py`
        to collect the repository data before categorization.
...
from lib.constants import REPO_ODS_FILE, MARKED_ODS_FILE, CATEGORIZED_OUTPUT, REPOS_SHEET_NAME, NPM_SHEET_NAME, \
    NPM_ORG_NAMES, NETLIFY_SHEET_NAME, NETLIFY_PREFIX_COLUMNS, ORG_NAMES
...
def determine_netlify_classification(row):
    ...
    Applies rules NL0–NL11 from Netlify.md in priority order and returns a
    ...
    Args:
        row (dict): Netlify site data. Relevant fields:
            ...
            - paused (bool/str): Whether the site is manually paused/disabled in Netlify.

    Returns:
        tuple[str, str]: (recommendation, reason) where recommendation is one of
            "Keep Auto Builds", "Disable Auto Builds", "Remove Project",
            "Nothing to Do", or "Manual Review".
    """
    ...
    paused = is_true(row.get("paused"))

    # Netlify.md Rule NL1 — Manual Review: site is manually paused/disabled
    if paused:
        return "Nothing to Do", "Site is manually paused/disabled in Netlify — confirm whether it should be reactivated or the project removed."

    # Netlify.md Rule NL2 — Nothing to Do: production site with a custom domain
    if not is_empty(custom_domain):
        return "Nothing to Do", "Site has a custom domain — it is serving production traffic."
    ...
    # Netlify.md Rule NL3 — Remove Project: orphaned + autogenerated/throwaway name
    ...
    # Netlify.md Rule NL4 — Manual Review: no linked repo (not caught by NL2)
    if is_empty(repo_url):
        return "Manual Review", "No repo linked — confirm whether the site has a purpose before removing."

    # Netlify.md Rule NL5 — Manual Review: linked repo is not in one of our GitHub organizations
    org_name = None
    if is_github_repo(repo_url):
        org_name, _ = split_github_repo(repo_url)

    is_our_org = org_name is not None and org_name.lower() in [org.lower() for org in ORG_NAMES]
    if not is_our_org:
        org_description = org_name if org_name else "unrecognized host"
        return "Manual Review", f"Linked repo's organization ({org_description}) is not one of the configured unfoldingWord GitHub organizations — confirm ownership before keeping or removing."

    # Netlify.md Rule NL6 — Manual Review: backing GitHub repo is archived
    if repo_archived:
        return "Manual Review", "Backing GitHub repo is archived — decide whether to keep as a static artifact, redirect, or remove."

    # Netlify.md Rule NL7 — Manual Review: site is in a non-organization Netlify account
    ...
    # Netlify.md Rule NL8 — Manual Review: site has never been published
    ...
    # Netlify.md Rule NL9 — Nothing to Do: published within the last 12 months
    if published_months is not None and published_months <= 12:
        return "Nothing to Do", f"Site was published within the last 12 months ({published_months} months ago)."

    # Netlify.md Rule NL10 — Disable Auto Builds: POC/demo/test/lab/playground/experiment/template name
    ...
    # Netlify.md Rule NL11 — Disable Auto Builds: published more than 18 months ago
    ...
    # Netlify.md Rule NL12 — Manual Review: published 12–18 months ago (borderline)
    ...
    # Netlify.md Rule NL13 — Manual Review: default
...
def main():
    ...
    if netlify_rows_:
        ...
        for column in ("repo archived", "paused"):
            if column in netlify_headers:
                netlify_headers.remove(column)

        auto_deploy_index = netlify_headers.index("auto_deploy")
        netlify_headers.insert(auto_deploy_index, "repo archived")
        netlify_headers.insert(auto_deploy_index + 1, "paused")

        # Add "Netlify Recommendation" and "Netlify Recommendation Reason"
        # columns before repo_branch
