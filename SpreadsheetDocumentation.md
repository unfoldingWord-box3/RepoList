...
| **Netlify Recommendation** | The Netlify lifecycle label assigned by `determine_netlify_classification()` in `CatagorizeRepos.py`. Sort order: `Remove Project`, `Disable Auto Builds`, `Manual Review`, `Keep Auto Builds`, `Nothing to Do`. |
| **Netlify Recommendation Reason** | A human-readable explanation of why the site received its recommendation label. |

`determine_netlify_classification()` applies rules NL0–NL11, plus NL3b, in priority order (first match wins).

| Label | Meaning |
|---|---|
| `Nothing to Do` | Site is manually paused/disabled in Netlify (rule NL0, checked before every other rule); or has a custom domain (rule NL1, serving production traffic); or was published within the last 12 months (rule NL7). |
| `Remove Project` | No linked repo, and the site name is Netlify-autogenerated or a throwaway placeholder (`trash`, `delete`). |
| `Manual Review` | No linked repo (not caught by the throwaway rule); linked repo's GitHub organization is not one of the configured unfoldingWord orgs (`ORG_NAMES`) (rule NL3b); backing GitHub repo is archived; site is in a non-organization Netlify account; site has never been published; last published 12–18 months ago (borderline); or did not match any other rule. |
...
| `Keep Auto Builds` | Listed in `NETLIFY_SORT_ORDER` and the function's docstring, but as of the current rules no `return` statement produces it — NL1 and NL7 (its former triggers) both return `Nothing to Do` now. Effectively dead. |

As with the GitHub/npm rules, the NL0–NL11 (and NL3b) rule definitions exist only as inline comments in `determine_netlify_classification()` — a prior `Netlify.md` was removed from the repo and the comments citing it were left in place.
