...
Produced by `CategorizeRepos.py`. Extends the **Repositories** sheet of `unfoldingword_repos.ods` with additional columns. Tag columns are prepended from `marked_repos.ods`, one derived column is inserted, and four classification columns are appended. The ODS also contains an **NPM Modules** sheet and a **Netlify** sheet.
...
The full rule definitions (rule IDs like `A3`, `NM2`) exist only as inline comments and docstrings in `determine_github_classification()` / `determine_npmjs_classification()` in `CategorizeRepos.py` — there is no standalone rules document; a prior `ClassificationRules.md` was removed from the repo and the comments citing it were left in place.

### NPM Modules sheet (in categorized_repos.ods)

A filtered view of npm-published repositories, columns reordered per `NPM_COLUMN_ORDER` in `CategorizeRepos.py`:
...
| **Netlify Recommendation** | The Netlify lifecycle label assigned by `determine_netlify_classification()` in `CategorizeRepos.py`. Sort order: `Remove Project`, `Disable Auto Builds`, `Manual Review`, `Keep Auto Builds`, `Nothing to Do`. |
| **Netlify Recommendation Reason** | A human-readable explanation of why the site received its recommendation label. |

`determine_netlify_classification()` applies rules NL1–NL13 in priority order (first match wins). The function's own docstring still says "Applies rules NL0–NL11" — that line is stale relative to the inline `# Netlify.md Rule NLxx` comments, which were renumbered to NL1–NL13.

| Label | Meaning |
|---|---|
| `Nothing to Do` | Site is manually paused/disabled in Netlify (rule NL1, checked before every other rule); or has a custom domain (rule NL2, serving production traffic); or was published within the last 12 months (rule NL9). |
| `Remove Project` | No linked repo, and the site name is Netlify-autogenerated or a throwaway placeholder (`trash`, `delete`) (rule NL3). |
| `Manual Review` | No linked repo (rule NL4, not caught by the throwaway rule); linked repo's GitHub organization is not one of the configured unfoldingWord orgs (`ORG_NAMES`) (rule NL5); backing GitHub repo is archived (rule NL6); site is in a non-organization Netlify account (rule NL7); site has never been published (rule NL8); last published 12–18 months ago, borderline (rule NL12); or did not match any other rule (rule NL13, default). |
| `Disable Auto Builds` | Site name suggests a POC/demo/test/lab/playground/experiment/template (rule NL10); or not published in over 18 months (rule NL11). |
| `Keep Auto Builds` | Listed in `NETLIFY_SORT_ORDER` and the function's docstring, but as of the current rules no `return` statement produces it — NL2 and NL9 (its former triggers) both return `Nothing to Do` now. Effectively dead. |

As with the GitHub/npm rules, the NL1–NL13 rule definitions exist only as inline comments in `determine_netlify_classification()` — a prior `Netlify.md` was removed from the repo and the comments citing it were left in place.
...
`marked_repos.ods` is a hand-modified version of `categorized_repos.ods` used to carry manual decisions into the categorized output. It is read by `CategorizeRepos.py` alongside `unfoldingword_repos.ods` and its tag columns are merged into `categorized_repos.ods` / `categorized_repos.csv`.
